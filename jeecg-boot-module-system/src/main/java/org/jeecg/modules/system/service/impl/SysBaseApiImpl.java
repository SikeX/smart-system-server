...
import org.jeecg.modules.villageHome.mapper.villageRelationMapper;
...
public class SysBaseApiImpl implements ISysBaseAPI {
    ...
    @Autowired
    private org.jeecg.modules.villageHome.mapper.villageRelationMapper villageRelationMapper;
    ...
    @Override
    public LoginUser getUserByIdNumber(String idNumber) {
        if (oConvertUtils.isEmpty(idNumber)) {
            return null;
        }
        LoginUser loginUser = new LoginUser();
...
//		QueryWrapper<SysDepart> queryWrapper = new QueryWrapper<>();
    ...
    public List<VillageRelationModel> getVillageRelation(String idnumber, String label) {
        List<VillageRelationModel> villageRelationList = new ArrayList<>();
        // 如果是户主
        if (label.equals("1")) {
            QueryWrapper<villageRelation> queryWrapper = new QueryWrapper<>();
            queryWrapper.eq("host_idnumber", idnumber);
            List<villageRelation> relationList = villageRelationMapper.selectList(queryWrapper);
            relationList.forEach(relation -> {
                VillageRelationModel villageRelationModel = new VillageRelationModel();
                BeanUtils.copyProperties(relation, villageRelationModel);
                villageRelationList.add(villageRelationModel);
            });
