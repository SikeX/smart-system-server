...
import org.jeecg.modules.SmartPaper.service.*;
...
public class SmartSurveyInfoController extends JeecgController<SmartExamInformation, ISmartSurveyInfoService> {
    ...
    @Autowired
    private ISmartMySurveyService smartMySurveyService;
    @Autowired
    ...
    public Result<?> queryPageMyList(SmartMySurveyVo smartMySurveyVo,
                                     @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
                                     @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,
                                     HttpServletRequest req) {
        // 1. 规则，下面是 以**开始
        ...
        QueryWrapper<SmartMySurveyVo> queryWrapper = QueryGenerator.initQueryWrapper(smartMySurveyVo, map);

        Page<SmartMySurveyVo> page = new Page<SmartMySurveyVo>(pageNo, pageSize);
        String oldExamName = smartMySurveyVo.getExamName();
        String examName = "";
        ...
        IPage<SmartMySurveyVo> pageList = smartMySurveyService.getMyAllSurvey(page,userId,examName);
        return Result.OK(pageList);
    }
